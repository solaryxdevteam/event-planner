...
const statusLabel = "Submit report";
...
function EventRow({ event }: { event: EventWithRelations }) {
  ...
  return (
    ...
    >
      ...
      <Badge
        variant="outline"
        className="shrink-0 text-xs border-amber-500/50 text-amber-700 bg-amber-50 dark:bg-amber-950/30 dark:text-amber-400"
      >
        {statusLabel}
      </Badge>
    ...
    <Card className="min-w-0 overflow-hidden p-3 sm:p-4 shadow-none gap-2">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 p-0 gap-2">
        ...
        <Button variant="ghost" size="sm" asChild className="shrink-0">
          <Link href="/dashboard/events?tab=past">View all</Link>
        </Button>
      </CardHeader>
      <CardContent className="p-0">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center rounded-lg border border-dashed bg-muted/30">
            ...
            <p className="text-sm text-muted-foreground">No events need your report</p>
