...
import { getColorClasses } from "@/lib/calendar/event-utils";
...
import { CalendarEvent, YearViewConfig } from "@/lib/calendar/types";
...
const DayCell = memo(({ day, events, isToday, onClick }: DayCellProps) => {
  ...
  const tooltipContent = useMemo(
    () => (hasDayEvents ? `${events.length} Event on ${format(day, "d MMMM yyyy")}` : format(day, "d MMMM yyyy")),
    ...
    return (
      <div
        className={cn(
          ...
          isCurrentMonth && yearViewConfig.highlightCurrentMonth && "border-blue-500 bg-blue-50/50 dark:bg-blue-950/10"
        )}
      >
          ...
          <div className="space-y-1 pt-1">
              ...
              .flatMap(([dateKey, events]) =>
                events.slice(0, 1).map((event) => {
                  ...
                  return (
                    <button
                      key={event.id}
                      className={cn("w-full truncate rounded-md px-2 py-1.5 text-left text-xs", colorClasses?.bg)}
            ...
            {eventCount > 3 && yearViewConfig.showMoreEventsIndicator && (
              <Button
                ...
                onClick={() => onMonthClick(new Date(month.getFullYear(), month.getMonth(), 30))}
              >
