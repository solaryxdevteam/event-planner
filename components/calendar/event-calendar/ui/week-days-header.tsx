    ...
    <div className={cn("flex w-full items-center justify-around", className)}>
      ...
      {reorderedDays.map((day, dayIndex) => {
        ...
        return (
          <motion.div
            key={dayIndex}
            className={cn("flex flex-1 flex-col items-center justify-center p-0 font-medium sm:p-2")}
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.3, delay: dayIndex * 0.05 }}
          >
            <span className="text-muted-foreground mb-1 text-xs sm:text-sm">{formatDate(day, "EEE", { locale })}</span>
            {showDayNumber && (
              <div className="">
                <span
                  className={cn(
