...
export function SearchYearPicker({ yearRange = 10, className = "", minYear, maxYear }: SearchYearPickerProps) {
      ...
      <PopoverTrigger asChild>
        ...
        >
          <Button
            ...
            role="combobox"
            aria-expanded={open}
            className={cn("w-[120px] justify-between text-sm font-normal", !year && "text-muted-foreground", className)}
            title="Select year"
          >
            ...
            <CommandGroup>
              <ScrollArea className="h-[200px]">
                {filteredYears.map((y) => (
                  ...
                  >
                    {y.label}
                    <Check className={cn("h-4 w-4", year === parseInt(y.value) ? "opacity-100" : "opacity-0")} />
                  </CommandItem>
                ))}
