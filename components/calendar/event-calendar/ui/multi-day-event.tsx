...
import { formatTimeDisplay, calculateDuration } from "@/lib/calendar/date-utils";
...
interface MultiDayEventSectionProps {
  ...
  daysInWeek: Date[];
  showEventDetail: (event: CalendarEvent) => void;
  multiDayRowHeight: number;
  isExpanded?: boolean;
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="mt-1 text-xs sm:truncate">{calculateDuration(event.startTime, event.endTime, "auto")}</div>
      </Button>
    </motion.div>
  );
};
...
export const MultiDayEventSection = memo(
  ({ rows, daysInWeek, showEventDetail, multiDayRowHeight, isExpanded = false }: MultiDayEventSectionProps) => {
