...
import React, { useState, useRef, useTransition, useMemo, useLayoutEffect } from "react";
...
export function EventCalendarTabs({ viewType, onChange, className = "", disabledViews = [] }: CalendarTabsProps) {
  const desktopButtonRefs = useRef<(HTMLButtonElement | null)[]>(new Array(tabsConfig.length).fill(null));
  const mobileButtonRefs = useRef<(HTMLButtonElement | null)[]>(new Array(tabsConfig.length).fill(null));
  ...
  const [visibleTabs, primaryTabs, secondaryTabs] = useMemo(() => {
    const visible = tabsConfig.filter((tab) => !disabledViews.includes(tab.value));
    return [visible, visible.slice(0, 2), visible.slice(2)];
  }, [disabledViews]);
  ...
  const [rects, setRects] = useState<{
    nav: DOMRect | null;
    mobileNav: DOMRect | null;
    selectedDesktop: DOMRect | null;
    selectedMobile: DOMRect | null;
    hoveredDesktop: DOMRect | null;
    hoveredMobile: DOMRect | null;
    dropdown: DOMRect | null;
  }>({
    nav: null,
    mobileNav: null,
  ...
  useLayoutEffect(() => {
    const id = requestAnimationFrame(() => {
      setRects({
        ...
        mobileNav: mobileNavRef.current?.getBoundingClientRect() ?? null,
        selectedDesktop: desktopButtonRefs.current[selectedTabIndex]?.getBoundingClientRect() ?? null,
        selectedMobile: mobileButtonRefs.current[primarySelectedTabIndex]?.getBoundingClientRect() ?? null,
        hoveredDesktop:
          hoveredTabIndex !== null
            ? (desktopButtonRefs.current[hoveredTabIndex]?.getBoundingClientRect() ?? null)
            : null,
        hoveredMobile:
          hoveredMobileTabIndex !== null
            ? (mobileButtonRefs.current[hoveredMobileTabIndex]?.getBoundingClientRect() ?? null)
            : null,
        dropdown: dropdownButtonRef.current?.getBoundingClientRect() ?? null,
      });
    });
    return () => cancelAnimationFrame(id);
  }, [viewType, selectedTabIndex, primarySelectedTabIndex, hoveredTabIndex, hoveredMobileTabIndex]);

  const { nav: navRect, mobileNav: mobileNavRect } = rects;
  const selectedDesktopRect = rects.selectedDesktop;
  const selectedMobileRect = rects.selectedMobile;
  const hoveredDesktopRect = rects.hoveredDesktop;
  const hoveredMobileRect = rects.hoveredMobile;
  const dropdownRect = rects.dropdown;
                ...
                <DropdownMenuContent align="start">
                  {daysOptions.map((option) => (
                    <DropdownMenuItem
                      ...
                      className={cn("cursor-pointer", storeDaysCount === option && "bg-muted font-medium")}
                    >
                      {option} days
                    </DropdownMenuItem>
                  ))}
