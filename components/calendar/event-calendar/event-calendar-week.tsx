...
import { formatDate, generateTimeSlots, isSameDay } from "@/lib/calendar/date-utils";
import { ScrollArea } from "@/components/ui/scroll-area";
import { CalendarEvent, HoverPositionType } from "@/lib/calendar/types";
...
export function EventCalendarWeek({ events, currentDate }: CalendarWeekProps) {
  ...
  const totalMultiDayRows = multiDayEventRows.length > 0 ? Math.max(...multiDayEventRows.map((r) => r.row)) + 1 : 1;

  const handleTimeHover = useCallback((hour: number) => {
    setHoverPosition((prev) => ({ ...prev, hour, minute: 0, dayIndex: -1 }));
  }, []);

  const handlePreciseHover = useCallback((event: React.MouseEvent<HTMLButtonElement>, hour: number) => {
          ...
          <div className="flex h-[64px] w-14 items-center justify-center sm:w-32">
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  <span className="sr-only">{isMultiDayExpanded ? "Collapse" : "Expand"} multi-day</span>
                  {isMultiDayExpanded ? <ChevronUp /> : <ChevronDown />}
                </Button>
              </TooltipTrigger>
              <TooltipContent>{isMultiDayExpanded ? "Collapse" : "Expand"} multi-day</TooltipContent>
            </Tooltip>
          </div>
          <div
            ...
            <div className="absolute inset-0">
              <div className="relative">
                {Array.from({
                  ...
                  <div key={`multi-day-row-${rowIndex}`} className="border-border flex h-16 border-t">
            ...
            <div className="relative flex-1 overflow-y-auto">
              ...
              <div className="pointer-events-none absolute inset-0">
                {singleDayEvents.map((event) => {
                  ...
                  const columnWidth =
                    (DAY_WIDTH_PERCENT + OVERLAP_FACTOR / position.totalColumns) / position.totalColumns;
                  const leftPercent =
