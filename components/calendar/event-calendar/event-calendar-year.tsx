...
import { CalendarViewType, CalendarEvent } from "@/lib/calendar/types";
...
export function EventCalendarYear({ events, currentDate }: CalendarYearProps) {
  const { openQuickAddDialog, openEventDialog, openDayEventsDialog, setView, viewSettings } = useEventCalendarStore(
    useShallow((state) => ({
  ...
  const handleQuickAdd = useCallback((date: Date) => openQuickAddDialog({ date }), [openQuickAddDialog]);
