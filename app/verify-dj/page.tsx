...
import { VerifyDjSummary } from "@/components/djs/VerifyDjSummary";
import type { VerifyDjInfo } from "@/lib/services/djs/dj-contact-verification.service";

function VerifyDjContent() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const token = searchParams.get("token");

  const [dj, setDj] = useState<VerifyDjInfo | null>(null);
  ...
  const loadDj = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/verify-dj?token=${encodeURIComponent(token)}`);
      if (res.status === 404) {
  ...
  const handleVerify = async () => {
    ...
    setVerifyError(null);
    try {
      const res = await fetch("/api/verify-dj", {
        method: "POST",
        ...
        <Card className="w-full max-w-md">
          <CardContent className="pt-6 text-center space-y-4">
            ...
            <p className="text-muted-foreground">
              Your email has been verified. You are now recognized as a verified DJ.
            </p>
            <Button onClick={handleRedirectToLogin} className="w-full">
              Go to login
            </Button>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-lg space-y-6">
        <Card>
          <CardHeader>
            <h1 className="text-xl font-semibold">Verify your DJ profile</h1>
            <p className="text-sm text-muted-foreground">
              Enter the 4-digit code sent to your email. The code is valid for 12 hours.
            </p>
          </CardHeader>
          <CardContent className="space-y-6">
            <VerifyDjSummary dj={dj} />
...
export default function VerifyDjPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen flex items-center justify-center p-4">
